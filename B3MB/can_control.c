...
typedef struct {
  ...
  uint8_t cmd;       //* The cmd code of the message
  uint8_t seq;       //* The current sequence number
  uint8_t len;       //* Message length, set in io_msg_init() before content is added
  bool err_flagged;  //* Indicates an error is already being sent
  bool in_progress;  //* Indicates the message is being built
} can_io_buf;
...
int32_t can_control_read(struct can_message *msg) {
  ...
  return can_async_read(&CAN_0, msg);
}
...
int32_t can_control_write(uint16_t ID, uint8_t *data, int nb) {
  ...
  can_tx_completed = false;
	return can_async_write(&CAN_0, &msg);
}
...
static void service_can_request(bool new_request) {
  if (cur_req.tx_blocked) {
    uint32_t rv = can_async_write(&CAN_0,&cur_req.omsg);
    switch (rv) {
      case ERR_NONE:
        cur_req.tx_blocked = false;
        break;
  ...
  do {
    ...
    { uint32_t rv;
      rv = can_async_write(&CAN_0, &cur_req.omsg);
      if (rv != ERR_NONE) {
        record_can_error(rv);
...
static void can_control_init(void) {
  ...
  //can_send_error_1(1, CAN_ERR_BAD_ADDRESS, 1);                  // Remove comment for testing B3MB CAN transmit bit rate
}
...
static subbus_cache_word_t can_cache[CAN_HIGH_ADDR-CAN_BASE_ADDR+1] = {
  ...
  { CAN_MAX_TXFR, 0, true,  false, false, false, false } // Offset 2: R: Maximum bytes not counting cmd bytes
};
...
subbus_driver_t sb_can = {
  ...
  can_cache,                    // cached memory label
  can_control_init,             // reset   function
  poll_can_control,             // poll    functin
  0,                            // Dynamic function
  false
};
