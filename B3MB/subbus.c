...
/* ************************************************************************************************
 * On_Off Control of 4x4 Full Cross-Point Battery Bus Switches 
 *    4 Battery Packs to 4 Loads via 8 switch control points
 *
 * Should be it's own module but I can't get it build that way ????? WTF (Litch)
 */


// Host Accessible Cache Address space for B3MB Digital I/O Status/Control Information
static subbus_cache_word_t on_off_cache[ON_OFF_HIGH_ADDR - ON_OFF_BASE_ADDR+1] = {
  { 0, 0, true,  false, true, false, true }, // Offset 0: R/W: batt_1 switch   On(1)/Off(0)
  { 0, 0, true,  false, true, false, true }, // Offset 1: R/W: batt_2 switch   On(1)/Off(0)
  { 0, 0, true,  false, true, false, true }, // Offset 2: R/W: batt_3 switch   On(1)/Off(0)
  { 0, 0, true,  false, true, false, true }, // Offset 3: R/W: batt_4 switch   On(1)/Off(0)
  { 0, 0, true,  false, true, false, true }, // Offset 4: R/W: load_1 switch   On(1)/Off(0)
  { 0, 0, true,  false, true, false, true }, // Offset 5: R/W: load_2 switch   On(1)/Off(0)
  { 0, 0, true,  false, true, false, true }, // Offset 6: R/W: load_3 switch   On(1)/Off(0)
  { 0, 0, true,  false, true, false, true }, // Offset 7: R/W: load_4 switch   On(1)/Off(0)
};
//   1          2        3          4          5          6            7
// value,     value,  boolean,   boolean,   boolean,   boolean,      boolean
// hardware,  host,   readable,  was_read,  writable,  was_written,  dynamic

// Dynamic (invoked by Host write to on_off_cache) switch control method
static void on_off_dynm(uint16_t addr) {
  if (sb_on_off.initialized) {
	bool sw_state = (on_off_cache[addr].wvalue == 1) ? true : false; 
    switch (addr) {
      case 0: gpio_set_pin_level(BATT1_ON, sw_state);  break;
      case 1: gpio_set_pin_level(BATT2_ON, sw_state);  break;
      case 2: gpio_set_pin_level(BATT3_ON, sw_state);  break;
      case 3: gpio_set_pin_level(BATT4_ON, sw_state);  break;
      case 4: gpio_set_pin_level(LOAD1_ON, sw_state);  break;
      case 5: gpio_set_pin_level(LOAD2_ON, sw_state);  break;
      case 6: gpio_set_pin_level(LOAD3_ON, sw_state);  break;
      case 7: gpio_set_pin_level(LOAD4_ON, sw_state);  break;	      
      default:
...
static void on_off_reset() {
	if (!sb_on_off.initialized) {
		sb_on_off.initialized = true;
	}
}

subbus_driver_t sb_on_off = {
	ON_OFF_BASE_ADDR, ON_OFF_HIGH_ADDR, // address range for ON-OFF Switch Control (8 switches)
	on_off_cache,                       // Host accessible cache structure
	on_off_reset,                       // reset function
	0,                                  // no poll function
	on_off_dynm,                        // Dynamic function invoked by CAN bus Write to any  control address
	false                               // initialized ?
};
