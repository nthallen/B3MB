...
static uint8_t  load_cnfg_reg[3] = { 0x01, 0xC5, 0x83};  // 0x01 = sets Addr_Pointer to configuration register
...
static uint8_t  load_ain_cmd[4] = { 0xC5, 0xD5, 0xE5, 0xF5 }; // 2nd of 3 bytes in load_cnfg_reg[3], choose which load_ain_x #
...
static void i2c_write(int16_t i2c_addr, const uint8_t *obuf, int16_t nbytes) {
  assert(load_txfr_complete, __FILE__, __LINE__);
  load_txfr_complete = false;
  i2c_m_async_set_slaveaddr(&I2C_LOAD, i2c_addr, I2C_M_SEVEN);
  io_write(I2C_LOAD_IO, obuf, nbytes);
}
...
static void i2c_read(int16_t i2c_addr, uint8_t *ibuf, int16_t nbytes) {
  ...
  load_txfr_complete = false;
  i2c_m_async_set_slaveaddr(&I2C_LOAD, i2c_addr, I2C_M_SEVEN);
  io_read(I2C_LOAD_IO, ibuf, nbytes);
}
...
void i2c_load_poll(void) {
  ...
  switch (load_state) {    // I2C enabled and no transfer in process so do current state's work, evaluate next state
    ...
    case load_config:                        // Write Config Reg, selects which channel to convert and starts conversion
      ...
      i2c_write(load_dev_addr, load_cnfg_reg, 3);
      // load_txfr_complete = false;
    ...
    case load_ptr_cnvr_reg:                  // Set Addr_Pointer to point to conversion data register
      load_dev_addr = (load_dev_x == 0) ? I2C_A_DEV_ADDR : I2C_B_DEV_ADDR;
      i2c_write(load_dev_addr, load_conver_reg, 1);
      // load_txfr_complete = false;
      // i2c_m_async_set_slaveaddr(&I2C_LOAD, load_dev_addr, I2C_M_SEVEN);
    ...
    case load_read_cnvrt:                    // Read converted data
      load_dev_addr = (load_dev_x == 0) ? I2C_A_DEV_ADDR : I2C_B_DEV_ADDR;
      i2c_read(load_dev_addr, load_ads_ibuf, 2);
      // load_txfr_complete = false;
      // i2c_m_async_set_slaveaddr(&I2C_LOAD, load_dev_addr, I2C_M_SEVEN);
    ...
    case load_cache_cnvrt:                   // Check read contents: MSByte's MSBit = status of conversion
      if (load_dev_x == 0) {
        ...
        sb_cache_update(i2c_load_cache, load_ain_x, (load_ads_ibuf[0] << 8) | load_ads_ibuf[1]);
        load_dev_x = 1;
        ...
        sb_cache_update(i2c_load_cache, load_ain_x + NUM_CHANNELS, (load_ads_ibuf[0] << 8) | load_ads_ibuf[1]);
        load_dev_x = 0;
