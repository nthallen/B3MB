...
status = sbsl.SBCAN_read_addrs(1, 64);
if status < 65280
  warning('\n One or More Load Switch Faults\n')
  pause
end
B3MB_cmd(sbsl, B3MB_ALL_BATTS_ON, 3); % B3MB_ALL_BATTS_ON cmd. 3 tries. 
pause(0.5)
Vana = getB3MB_ana(sbsl, 23, 20, .1);  % Take Zero-Current Voltage readings
Vzero = mean(Vana,2)  % Display for log
